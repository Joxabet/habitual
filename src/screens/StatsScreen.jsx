...
export function StatsScreen({ habits, checkins }) {
  ...
  const LEVEL_COLORS = ['bg-neutral-800', 'bg-green-900/60', 'bg-green-600/60', 'bg-green-500']
  const statCards = [
    { icon: Flame,       label: 'Best streak',    value: stats.bestStreak, unit: 'days',     color: 'text-amber-400' },
    { icon: TrendingUp,  label: 'This week',       value: stats.weekRate+'%', unit: 'completion', color: 'text-green-400' },
    { icon: CheckCircle, label: 'Total check-ins', value: stats.totalCheckins, unit: 'logged',color: 'text-blue-400' },
    { icon: Calendar,    label: 'Active habits',   value: habits.length,   unit: 'habits',   color: 'text-purple-400' },
  ]
  ...
  return (
    <div className="flex-1 overflow-y-auto scrollbar-hide">
      ...
      <div className="grid grid-cols-2 gap-3 px-4 mb-6">
        {statCards.map(({ icon: Icon, label, value, unit, color }) => (
          <div key={label} className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4">
            ...
            <div className="text-2xl font-medium font-mono">{value}</div>
      ...
      <div className="px-4 mb-6">
        <p className="text-[10px] uppercase tracking-widest text-neutral-600 mb-3">Activity — last 12 weeks</p>
      ...
      {habits.length > 0 && (
        <div className="px-4 mb-6">
          <p className="text-[10px] uppercase tracking-widest text-neutral-600 mb-3">Habit streaks</p>
          <div className="flex flex-col gap-2">
            {habits.map(h => {
              const streak = calcStreak(checkins[h.id] || [], h)
              const pct = Math.min(100, Math.round((streak / 30) * 100))
              return (
                <div key={h.id} className="bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-3">
      ...
      <div className="px-4 mb-8">
        <p className="text-[10px] uppercase tracking-widest text-neutral-600 mb-3">Milestones</p>
            ...
            return (
              <div key={m.id} className={`p-3 rounded-2xl border flex items-center gap-3 ${
                ...
                <div className="min-w-0">
                  <p className={`text-xs font-medium leading-tight ${earned ? 'text-neutral-200' : 'text-neutral-600'}`}>
                    {m.label}
                  </p>
                  <p className={`text-[10px] mt-0.5 leading-tight ${earned ? 'text-neutral-500' : 'text-neutral-700'}`}>
                    {m.desc}
                  </p>
                </div>
                {!earned && <Lock size={11} className="text-neutral-700 flex-shrink-0 ml-auto" />}
              </div>
            )
          })}
        </div>
