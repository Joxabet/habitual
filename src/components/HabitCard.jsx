...
export function HabitCard({ habit, checkins, isDone, onToggle, onArchive, onDelete, dragHandleProps }) {
  ...
  if (showActions) {
    return (
      <div className="flex items-center gap-2 px-1 py-1 rounded-2xl bg-neutral-100 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800">
        <div className="flex-1 px-3 py-2">
          <p className="text-sm text-neutral-500 dark:text-neutral-400">{ICONS[habit.icon] || '✨'} {habit.name}</p>
        </div>
        <button
        ...
        >
          ...
          <span className="text-[10px] text-amber-500 dark:text-amber-400">Archive</span>
        </button>
        <button
        ...
        >
          ...
          <span className="text-[10px] text-red-500 dark:text-red-400">Delete</span>
        </button>
        <button
          onClick={() => setShowActions(false)}
          className="px-3 py-2 text-xs text-neutral-500 dark:text-neutral-600 active:text-neutral-400"
        >
          Cancel
        </button>
      </div>
  ...
  return (
    <div
      className={`flex items-center gap-3 p-4 rounded-2xl border transition-colors ${
        isDone
          ? 'bg-white dark:bg-neutral-900 border-green-500/40 dark:border-green-500/30'
    ...
    >
      ...
      <div
        {...dragHandleProps}
        className="flex-shrink-0 text-neutral-300 dark:text-neutral-700 touch-none cursor-grab active:cursor-grabbing p-1 -ml-1"
      ...
      <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center text-lg ${
        isDone ? 'bg-green-500/10' : 'bg-neutral-100 dark:bg-neutral-800'
      }`}>
        {ICONS[habit.icon] || '✨'}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <p className={`font-medium text-sm leading-tight ${
          isDone
            ? 'text-neutral-400 dark:text-neutral-500 line-through decoration-neutral-300 dark:decoration-neutral-700'
        ...
        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
          {streak > 0 && (
            <span className="flex items-center gap-1 text-xs text-amber-500 dark:text-amber-400 font-mono">
          ...
          {target && (
            <div className="flex items-center gap-1">
              {Array.from({ length: target }).map((_, i) => (
                <div key={i} className={`w-3.5 h-1.5 rounded-full ${
                  i < weekCount ? 'bg-green-500' : 'bg-neutral-200 dark:bg-neutral-700'
                }`} />
              ))}
              <span className="text-xs text-neutral-400 dark:text-neutral-500 ml-1 font-mono">{weekCount}/{target}</span>
            </div>
          )}
          {!streak && !target && (
