...
export function OnboardingScreen({ onComplete }) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-neutral-950 px-6 pt-16 pb-10">
      {/* Progress dots */}
        ...
        {STEPS.map((_, i) => (
          <div
            ...
            className={`rounded-full transition-all duration-300 ${
              i === step ? 'w-6 h-2 bg-green-400' : 'w-2 h-2 bg-neutral-800'
            }`}
          />
        ))}
      </div>
      ...
      <div className="flex-1 flex flex-col items-center text-center">
        ...
        <h1 className="text-2xl font-medium text-neutral-100 mb-4 leading-tight">
          {current.title}
        </h1>
      ...
      <button
        ...
        className="w-full py-4 rounded-2xl bg-green-500 text-neutral-950 font-medium text-base flex items-center justify-center gap-2 active:scale-98 transition-transform"
      >
        {current.cta}
      ...
      {!isLast && (
        <button
          onClick={onComplete}
          className="mt-4 text-sm text-neutral-700 py-2 w-full text-center"
        >
          Skip
        </button>
      )}
